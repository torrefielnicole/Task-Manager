...
.task-item { display:flex;align-items:center;gap:12px;padding:11px 13px;border-radius:11px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.04);margin-bottom:8px;position:relative;overflow:hidden;transition:transform .18s,background .18s,box-shadow .18s;cursor:pointer; }
...
.task-item:hover { transform:translateX(4px);background:rgba(255,255,255,.08);box-shadow:0 6px 20px rgba(0,0,0,.2); }
...
.badge-pending   { background:rgba(255,184,48,.15);color:var(--warn); }
.badge-completed { background:rgba(0,229,160,.15);color:var(--done); }
.badge-urgent    { background:rgba(255,82,82,.15);color:var(--danger); }
...
.dl-item { display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.05);transition:background .15s; }
.dl-item:last-child { border-bottom:none; }
.dl-item:hover { background:rgba(255,255,255,.03);border-radius:8px;padding-left:6px; }
...
<main class="main">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card sc-red">
            ...
            <div class="stat-num c-red" data-target="<?= count($dueSoon) ?>"><?= count($dueSoon) ?></div>
        </div>
    </div>
    ...
    <div class="two-col">
        ...
        <div class="task-panel">
            <div class="panel-title">📌 All Academic Tasks</div>
            <?php if ($total > 0): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="empty-state"><span>📭</span>No academic tasks yet</div>
            <?php endif; ?>
        </div>
        ...
        <div class="deadline-panel">
            ...
            <div class="dl-item">
                <div>
                    <div class="dl-name"><?= htmlspecialchars($t['task_name']) ?></div>
                    <div class="dl-sub">📅 <?= $t['due_date'] ?></div>
                </div>
...
new Chart(document.getElementById('acadChart'),{
    ...
    data:{
        ...
        datasets:[{
            data:[<?= $completed ?>,<?= $pending ?>,<?= count($dueSoon) ?>,<?= $total ?>],
            backgroundColor:['rgba(0,229,160,.25)','rgba(255,184,48,.25)','rgba(255,82,82,.25)','rgba(79,195,247,.25)'],
            borderColor:['#00e5a0','#ffb830','#ff5252','#4fc3f7'],
            borderWidth:2,borderRadius:10,
        }]
    },
    options:{
        responsive:true,
        plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(20,25,80,.95)',borderColor:'rgba(79,195,247,.3)',borderWidth:1,titleColor:'#fff',bodyColor:'rgba(255,255,255,.7)'}},
        scales:{
            x:{grid:{color:'rgba(255,255,255,.05)'},ticks:{color:'rgba(255,255,255,.5)',font:{size:12}}},
            y:{grid:{color:'rgba(255,255,255,.05)'},ticks:{color:'rgba(255,255,255,.5)',font:{size:12}},beginAtZero:true}
        }
    }
});
</script>
