...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'edit') {
        ...
        if ($id && $name !== '') {
            $s = $conn->prepare("UPDATE task SET task_name=?, due_date=?, status=? WHERE task_id=? AND category='project'");
    ...
    if ($action === 'delete') {
        ...
        if ($id) {
            $s = $conn->prepare("DELETE FROM task WHERE task_id=? AND category='project'");
